...
def handle_document(update, context):
  ...
  for phone in phones:
      exists_phones.append(validator.check_phone_existence_from_db())

  for email in emails:
      exists_emails.append(validator.check_emails_existence_from_db())

  # Формирование ответа
  response = "Найденные данные:\n\n"
  if phones:
    response += "**Номера телефонов:**\n" + "\n".join(map(str, exists_phones)) + "\n\n"
  if emails:
    response += "**Email адреса:**\n" + "\n".join(map(str, exists_emails))
